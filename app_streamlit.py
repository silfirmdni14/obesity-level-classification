...
with st.sidebar:
    ...
    <div class="guide-card">
        ...
        <ul>
            ...
            <li>Tinggi dalam meter (mis. 1.72)</li>
...
tab1, tab2, tab3,tab4 = st.tabs([ "📊  Informasi", "📥  Prediksi Obesitas", "📈 Analisis Data", "👤  About Me"])
# TAB 1 — INFORMASI
with tab1:
    # ---------- HERO ----------
    st.markdown(f"""
...
with tab2:
    ...
    with col_a:
        with st.container(border=True):
            ...
            age            = st.number_input("Age (Umur)", min_value=0, value=25)
            height         = st.number_input("Height (Tinggi Badan, dalam meter)", min_value=0.0, value=1.70, step=0.01)
            weight         = st.number_input("Weight (Berat Badan, dalam kg)", min_value=0.0, value=65.0, step=0.5)
    ...
    with col_b:
        with st.container(border=True):
            ...
            fcvc = st.slider("Konsumsi Sayur (1=jarang, 3=sering)", 1.0, 3.0, 2.0)
            ncp  = st.slider("Jumlah Makan Utama / hari", 1, 4, 3)
    ...
    with col_c:
        with st.container(border=True):
            ...
            faf    = st.slider("Frekuensi Olahraga / minggu", 0.0, 3.0, 1.5)
            tue    = st.slider("Waktu Gadget (jam/hari)", 0.0, 2.0, 1.0)
            mtrans = st.selectbox("Transportasi", ["Walking", "Bike", "Motorbike", "Public_Transportation", "Car"])
    with col_d:
        with st.container(border=True):
    ...
    if predict_clicked:
        data_baru = pd.DataFrame(
            [[gender, age, height, weight, family_history, favc, fcvc, ncp, caec, ch2o, faf, tue, calc, mtrans]],
